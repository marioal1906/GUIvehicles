...
parser.add_argument("interpolation3", type=float, help="The width of the part.")
...
interpolation3 = args.interpolation3
...
for draw in range(n_draws):
    ...
    if draw==0: # front yz
        ...
        data=data[dataIndex,:]
        data=data/100
        sizeShortData=sizeShortData+1
        print(sizeShortData)

        plt.plot(data[:,0],data[:,1],'-')
        plt.axis('equal')
        plt.title("Silohuette data")
        plt.tight_layout()
        plt.show()
        ...
        geoList0 = []
        App.ActiveDocument.Sketch0.addGeometry(Part.Circle(App.Vector(data[0,0],data[0,1],0),App.Vector(0,0,1),10),True)
        ...
        for i in range(sizeShortData-1):
            App.ActiveDocument.Sketch0.addGeometry(Part.Circle(App.Vector(data[i+1,0],data[i+1,1],0),App.Vector(0,0,1),10),True)
            App.ActiveDocument.Sketch0.addConstraint(Sketcher.Constraint('Equal',0,i+1))
            geoList0.append(App.Vector(data[i+1,0],data[i+1,1]))
            #print(i)
        App.ActiveDocument.Sketch0.addGeometry(Part.BSplineCurve(geoList0,None,None,False,3,None,False),False)
        #print(geoList0)

        conList = []
        for i in range(sizeShortData):
            #print(i)
            conList.append(Sketcher.Constraint('InternalAlignment:Sketcher::BSplineControlPoint',i,3,sizeShortData,i))
    ...
    if draw==1: # side xy
        ...
        App.ActiveDocument.recompute()
        geoList1 = []
        App.ActiveDocument.Sketch1.addGeometry(Part.Circle(App.Vector(data[0,0],data[0,1],0),App.Vector(0,0,1),10),True)
        ...
        for i in range(sizeShortData-1):
            App.ActiveDocument.Sketch1.addGeometry(Part.Circle(App.Vector(data[i+1,0],data[i+1,1],0),App.Vector(0,0,1),10),True)
            App.ActiveDocument.Sketch1.addConstraint(Sketcher.Constraint('Equal',0,i+1))
            geoList1.append(App.Vector(data[i+1,0],data[i+1,1]))
            #print(i)
        App.ActiveDocument.Sketch1.addGeometry(Part.BSplineCurve(geoList1,None,None,False,3,None,False),False)
        #print(geoList1)
    ...
    if draw==2: # top xz
        ...
        geoList2 = []
        App.ActiveDocument.Sketch2.addGeometry(Part.Circle(App.Vector(data[0,0],data[0,1],0),App.Vector(0,0,1),10),True)
        ...
        for i in range(sizeShortData-1):
            App.ActiveDocument.Sketch2.addGeometry(Part.Circle(App.Vector(data[i+1,0],data[i+1,1],0),App.Vector(0,0,1),10),True)
            App.ActiveDocument.Sketch2.addConstraint(Sketcher.Constraint('Equal',0,i+1))
            geoList2.append(App.Vector(data[i+1,0],data[i+1,1]))
            #print(i)
        App.ActiveDocument.Sketch2.addGeometry(Part.BSplineCurve(geoList2,None,None,False,3,None,False),False)
        #print(geoList1)
        conList = []
        for i in range(sizeShortData):
            #print(i)
